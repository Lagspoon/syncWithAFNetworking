...
- (void)downloadDataForRegisteredObjects:(BOOL)useUpdatedAtDate toDeleteLocalRecords:(BOOL)toDelete {
    ...
    } completionBlock:^(NSArray *operations)
    {
        ...
        if (!toDelete) {
            // Process JSON records for Deletion
...
#pragma mark - IMPORT INTO CORE DATA
////////////////////////////////////////////////////
...
- (void)postLocalObjectsToServer {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        for (NSManagedObject *objectToCreate in objectsToCreate) {
            ...
            //
            NSMutableURLRequest *request = [[SYHTTPClient sharedClient] POSTRequestForClass:className parameters:jsonString];
            AFHTTPRequestOperation *operation = [[SYHTTPClient sharedClient].requestOpManager HTTPRequestOperationWithRequest:request
                                                                                                        success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                                                                                            //
...
- (void)deleteObjectsOnServer {
    ...
    for (NSString *className in self.registeredClassesToSync) {
        ...
        for (NSManagedObject *objectToDelete in objectsToDelete) {
            ...
            //
            NSMutableURLRequest *request = [[SYHTTPClient sharedClient]
                                            DELETERequestForClass:className
                                            forObjectWithId:[objectToDelete valueForKey:@"objectId"]];

            AFHTTPRequestOperation *operation = [[SYHTTPClient sharedClient].requestOpManager HTTPRequestOperationWithRequest:request success:^(AFHTTPRequestOperation *operation, id responseObject) {

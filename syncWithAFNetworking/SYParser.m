...
- (BOOL) saveObjectsDownloaded {
    BOOL success = NO;
    success = [self.delegateCoreData saveBackgroundContext];
    [self.delegateSyncEngine managedObjectContextUpdated];
    return success;
}
- (void) resetObjectsDownloaded {
    [self.delegateCoreData.backgroundManagedObjectContext reset];
}
